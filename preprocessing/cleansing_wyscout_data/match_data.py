...
def get_all_formations(matches):
    ...
    all_formations["minuteStart"] = np.where(
        all_formations["substituteIn"].isnull(), 0, all_formations["substituteIn"]
    )
    all_formations["minuteEnd"] = np.where(
        all_formations["substituteOut"].isnull(), 90, all_formations["substituteOut"]
    )
    ...
    all_formations["minuteEnd"] = np.where(
        (all_formations["lineup"] == 0) & (all_formations["substituteIn"].isnull()),
        0,
        all_formations["minuteEnd"],
    )

    # compute the minutes played
    all_formations["minutesPlayed"] = (
        all_formations["minuteEnd"] - all_formations["minuteStart"]
    )
