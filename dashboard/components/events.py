...
class Events:
    ...
    def compute_centroids(self, match_id, team_id, relevant_events=["Pass", "Shot", "Duel"]):

        df_events = self.get_events(match_id, relevant_events)
        df_events_team = df_events[df_events["teamId"] == team_id]

        df_centroids = df_events_team. \
            groupby("playerId"). \
            agg(centerX=("posBeforeX", "mean"),
                centerY=("posBeforeY", "mean")).reset_index()
        return df_centroids

    def prepare_heatmap(self, player_id, match_id=None, event_names=["Pass", "Shot", "Duel"]):

        if match_id is None:
            match_id = "all"

        df_events = self.get_events(matches=match_id, event_names=event_names, players=player_id)
        ...
        df_pos_player = df_events.groupby(["groupX", "groupY"]). \
            agg(nbEvents=("playerId", "count")).reset_index()
        df_all_pos = pd.DataFrame([(x, y) for x in range(10) for y in range(10)], columns=["groupX", "groupY"])
